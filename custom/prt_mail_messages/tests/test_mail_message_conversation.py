...
class TestMailMessageConversation(common.TransactionCase):
    ...
    TEST 5 : Get or create new partner by Email
        ...
        [Get or create partner id by email 'Partner Example <partner.example@example.com>']
        [Get partner by email 'partner.example@example.com']
    ...
    TEST 6 : Get conversations name get
        ...
        - result must be equal to [(conversation.id,  "Test Conversation #1 - env.user.name")]
    ...
    def test_user_is_participant(self):
        ...
        self.assertTrue(
            is_participant,
            msg=f"Demo User is participant in #{self.cetmix_conversation_1.id} conversation.",
        )
        self.cetmix_conversation_1.write({"partner_ids": [(6, 0, [])]})
        with self.assertRaises(exceptions.AccessError):
            _ = self.cetmix_conversation_1.with_user(
                self.res_users_demo.id
            ).is_participant
